...
class UserController extends Controller {
	...
	protected static $facebook_posts = [];
	...
	public static function getFacebookPosts( $fb, $access_token ) {

		$since = strtotime("-1 year", time());

		try {
			$posts = $fb->get( '/me/feed?limit=500&since=' . $since , $access_token );
		}
		...
		if( isset( $postsObject['data'] ) && count( $postsObject['data'] ) ) {
			foreach ( $postsObject['data'] as $key => $value) {
				self::$facebook_posts[] = $value;
			}
		}

		if( isset( $postsObject['paging']['next'] ) && $postsObject['paging']['next'] != '' ) {

			self::getFacebookPostsPagination( $postsObject['paging']['next'] );
		}

		return 1;
	}

	public static function getFacebookPostsPagination( $url ) {
		...
		$postsObject = json_decode( $output, true );
	...
	public static function getFacebookUserDetails( $user, $detail ) {
		...
		self::getFacebookPosts( $fb, $access_token );

		// $since = strtotime("-1 year", time());

		// try {
		// 	$posts = $fb->get( '/me/feed?since=' . $since , $access_token );
		// }
		// catch (\Facebook\Exceptions\FacebookResponseException $e) {
		// 	// echo 'Graph returned an error: ' . $e->getMessage();
		// 	// exit;
		...
		$posts_count = count( self::$facebook_posts );
		$post_array = self::$facebook_posts;
			...
			for ($i = 0; $i < $posts_count; $i++) {
				...
				try {
					...
					if( $friendsCount > 0 ) {
						...
						$postsEngagementArray[$i] = $engagementTemp / $friendsCount;
					}
				}
				catch (\Facebook\Exceptions\FacebookResponseException $e) {
		...
		if( $posts_count > 0 ) {
			...
			$result['average_engagement'] = ( $result['average_likes'] + $result['average_comment'] + $result['average_shares'] ) / $posts_count;
		}
		else {
	...
	public static function getPlaylistItems($playlistId, $youtube_api_key,$nextPageToken = '') {
		...
		if(isset($playlistItems->items) && count($playlistItems->items)) {
			...
			$beforeYear = strtotime("-1 year", time());
	...
	public static function getGoogleUserDetails($user, $detail) {
			...
			foreach ($channels->items as $key => $val) {
				...
				if(isset($val->contentDetails->relatedPlaylists->uploads) && $val->contentDetails->relatedPlaylists->uploads!='' ){
					...
					if( self::$youtube_total_videos > 0 ) {
						$youtubeDetails['average_views'] = self::$youtube_total_views/self::$youtube_total_videos;
						$youtubeDetails['average_comment'] = self::$youtube_total_comment/self::$youtube_total_videos;
						$youtubeDetails['average_likes'] = self::$youtube_total_likes/self::$youtube_total_videos;
						$youtubeDetails['average_dislikes'] = self::$youtube_total_dislikes/self::$youtube_total_videos;
					}
		...
		$youtubeDetails['average_engagement'] = 0;
		if( $youtubeDetails['follows_count'] > 0 ) {
			$youtubeDetails['average_engagement'] = ( $youtubeDetails['average_views'] + $youtubeDetails['average_likes'] + $youtubeDetails['average_views'] ) / $youtubeDetails['follows_count'];
		}
